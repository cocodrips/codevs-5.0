...
public:
    ...
    bool isRock() const { return kind == CELL_ROCK; }
...
public:
    ...
    vector<vector<Cell> > getCopiedField(){
        return vector<vector<Cell> >(field);
...
bool canMove(vector<vector<Cell> > &field,int x, int y, int dir) {
    if (field[y][x].isEmpty()) {
        return true;
    }
    if (field[y][x].isRock()) {
        if (field[y + dy[dir]][x + dx[dir]].isEmpty()) {
            field[y + dy[dir]][x + dx[dir]] = CELL_ROCK;
        };
    }
    return false;
}
...
MinDist getMinDist(const State &st, int x, int y, int dir) {
    vector<vector<Cell> > field = st.getCopiedField();
    if (!canMove(field, x, y, dir)) {
...
string thinkByNinjaId(int id) {
    ...
    for (int dir = 0; dir < 5; dir++) {
        int nx = myState.ninjas[id].x + dx[dir];
        int ny = myState.ninjas[id].y + dy[dir];
        dists.push_back(getMinDist(myState, nx, ny, dir));
